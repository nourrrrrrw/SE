...
// Helper for rendering dot scale
        ...
        <label key={v} style={{ cursor: 'pointer' }}>
          ...
          <span
            style={{
              ...
              height: 18,
              borderRadius: '50%',
              background: value >= v ? '#F08F36' : '#C0CEDB',
              border: value >= v ? '2px solid #F08F36' : '2px solid #C0CEDB',
              marginRight: 2,
          ...
          {currentInternship && (
            <div style={cardStyle}>
              ...
              <div
                ...
                onClick={() => setActiveCurrentInternship(activeCurrentInternship === 'current-' + currentInternship.id ? null : 'current-' + currentInternship.id)}
              >
                <div style={{ fontWeight: 600, color: '#234B73', fontSize: 18 }}>{currentInternship.company}</div>
                <div style={{ color: '#8C8C8C', fontSize: 16 }}>{currentInternship.position}</div>
                <div style={{ color: '#234B73', fontWeight: 500, fontSize: 15 }}>
                  {new Date(currentInternship.startDate).toLocaleDateString()} - {new Date(currentInternship.endDate).toLocaleDateString()}
                </div>
              </div>
              {activeCurrentInternship === 'current-' + currentInternship.id && (
                <div style={{ background: '#F6F8FA', borderRadius: 8, margin: '18px 0', padding: 24 }}>
                  <div style={{ marginBottom: 12 }}><b>Company:</b> {currentInternship.company}</div>
                  <div style={{ marginBottom: 12 }}><b>Job Title:</b> {currentInternship.position}</div>
                  <div style={{ marginBottom: 12 }}><b>Duration:</b> {new Date(currentInternship.startDate).toLocaleDateString()} - {new Date(currentInternship.endDate).toLocaleDateString()}</div>
                  <div style={{ marginBottom: 12 }}><b>Paid:</b> {currentInternship.paid ? 'Paid' : 'Unpaid'}</div>
                  {currentInternship.paid && (
                    <div style={{ marginBottom: 12 }}><b>Expected Salary:</b> ${currentInternship.expectedSalary} / month</div>
                  )}
                  <div style={{ marginBottom: 12 }}><b>Skills Required:</b> {currentInternship.skills.join(', ')}</div>
                  <div style={{ marginBottom: 12 }}><b>Job Description:</b> {currentInternship.jobDescription}</div>
                  {/* Progress Bar */}
                  <div style={{ marginTop: 24, marginBottom: 8 }}>
                    <b>Progress:</b>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
                      <div style={{ width: 200, height: 10, background: '#C0CEDB', borderRadius: 5, overflow: 'hidden' }}>
                        <div style={{ width: `${currentInternship.progress}%`, height: '100%', background: '#F08F36', borderRadius: 5, transition: 'width 0.3s' }} />
                      </div>
                      <span style={{ color: '#234B73', fontWeight: 600 }}>{currentInternship.progress}%</span>
                    </div>
                  </div>
                </div>
              )}
                ...
                {filteredInternships.map(internship => {
                  ...
                  return (
                    <React.Fragment key={uniqueKey}>
                      ...
                      >
                        <div style={{ fontWeight: 600, color: '#234B73', fontSize: 18 }}>{internship.company}</div>
                        <div style={{ color: '#8C8C8C', fontSize: 16 }}>{internship.position}</div>
                        <div style={{ color: '#234B73', fontWeight: 500, fontSize: 15 }}>
                      ...
                      {activeListInternship === uniqueKey && (
                        <div style={{ background: '#F6F8FA', borderRadius: 8, margin: '18px 0', padding: 24 }}>
                          ...
                          <div style={{ marginBottom: 12 }}><b>Duration:</b> {new Date(internship.startDate).toLocaleDateString()} - {new Date(internship.endDate).toLocaleDateString()}</div>
                          ...
                          {internship._type === 'past' && (
                            <div style={{ marginTop: 32, paddingTop: 18, borderTop: '1px solid #C0CEDB' }}>
                                    ...
                                    return (
                                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <span style={{
                                          ...
                                          fontWeight: 700,
                                          fontSize: 14,
                                          background: report.status === 'Flagged' ? '#FEF3C7' : report.status === 'Rejected' ? '#FEE2E2' : '#E9D5FF',
                                          color: report.status === 'Flagged' ? '#92400E' : report.status === 'Rejected' ? '#991B1B' : '#7C3AED',
                                          display: 'inline-flex',
                                          alignItems: 'center',
                                          gap: 6,
                                          minWidth: 80,
                                          ...
                                          {report.status === 'Flagged' && <span style={{ fontSize: 16, marginRight: 2 }}>⚠️</span>}
                                          {report.status === 'Rejected' && <span style={{ fontSize: 16, marginRight: 2 }}>⛔</span>}
                                          {report.status === 'Finalized' && <span style={{ fontSize: 16, marginRight: 2 }}>✔️</span>}
                              ...
                              >
                                {reports[getInternshipKey(internship)] ? (reports[getInternshipKey(internship)].finalized ? 'View Report' : 'Edit Report') : 'Create Report'}
                              </button>
                            </div>
                          )}
                        </div>
      ...
      {showEvalModal && (
        <>
          ...
          >
            ...
            >
              <div style={{ color: '#234B73', fontWeight: 500, fontSize: 16 }}>Rate the following (1 = Poor, 5 = Excellent):</div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                ...
                >
                  {evaluations[evalModalInternshipId] ? 'Update' : 'Save'}
                </button>
                <button
                  type="button"
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
                  <span style={{
                    ...
                    fontWeight: 700,
                    fontSize: 15,
                    background: !reportDraft.status ? '#DBEAFE' : reportDraft.status === 'Flagged' ? '#FEF3C7' : reportDraft.status === 'Rejected' ? '#FEE2E2' : (reportDraft.status === 'Accepted' || reportDraft.status === 'Approved') ? '#D1FAE5' : '#E9D5FF',
                    color: !reportDraft.status ? '#1E40AF' : reportDraft.status === 'Flagged' ? '#92400E' : reportDraft.status === 'Rejected' ? '#991B1B' : (reportDraft.status === 'Accepted' || reportDraft.status === 'Approved') ? '#065F46' : '#7C3AED',
